...
class ComplexTensor:
    def __init__(self, real: Union[torch.Tensor, numpy.ndarray],
                 imag: Union[torch.Tensor, numpy.ndarray]=None):
        if imag is None:
    ...
    def inverse(self):
        ...
        o_real = (a.real + torch.matmul(torch.matmul(a.imag, a.real.inverse()),
                                        a.imag)).inverse()
        o_imag = -torch.matmul(o_real, torch.bmm(a.imag, a.real.inverse()))
